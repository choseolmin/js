describe('Object에 대해서 학습합니다.', function () {
  ...
  */
  ...
  it("'this'는 method를 호출하는 시점에 결정됩니다.", function () {
    ...
    expect(megalomaniac.calculateAge(currentYear)).to.equal(55);

    megalomaniac.birthYear = 2000;
    expect(megalomaniac.calculateAge(currentYear)).to.equal(25);

    megalomaniac.changeBirthYear(2010);
    expect(megalomaniac.calculateAge(currentYear)).to.equal(15);
  ...
  it('Object를 함수의 인자로 전달할 경우, reference가 전달됩니다.', function () {
    ...
    expect(copiedObj.henchwoman).to.equal('Harley');
